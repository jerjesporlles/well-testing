...
def ingreso_datos_drawdown():

	col1,col2 = st.beta_columns(2)
	with col1 :
		qo = st.number_input('Input qo',min_value=0.00, max_value=None, value=2700.00,  step=1.00 )
		
	with col2 :
		pwf = st.number_input('Input Pwf(t=0)', min_value=0.00, max_value=None, value=11348.00,  step=1.00 )
	return qo,pwf

def Drawdown_Test_Sequence():
	st.sidebar.info("Bienvenido al módulo Drawdown Test Sequence") #Amarillo
	...
	if archivo_excel is None:
		...
		with st.beta_expander('Drawdown Test Sequence'):
			qo,pwf = ingreso_datos_drawdown()


		with st.beta_expander('Cartesian analysis '):
			col5, col6, col7 = st.beta_columns(3)
			with col5:
				df_cartesian_analysis = df[['t, hr','pwf, psia']]
				st.write(df_cartesian_analysis)

			with col6 :
				t1 = st.number_input('Ingrese el tiempo 1 de la base de datos',min_value=0.00, max_value=None, value=0.50,  step=0.1)
				t2 = st.number_input('Ingrese el tiempo 2 de la base de datos',min_value=0.00, max_value=None, value=0.25,  step=0.1)
			with col7 :
				p1 = st.number_input('Ingrese la presión 1 de la base de datos',min_value=0.00, max_value=None, value=10630.00,  step=0.1)
				p2 = st.number_input('Ingrese la presión 2 de la base de datos',min_value=0.00, max_value=None, value=10989.00,  step=0.1)

		with st.beta_expander('Pressure drop at the start of the test, pi(t=0)'):
			col8, col9, col10 = st.beta_columns(3)
			with col8 :
				dP = st.number_input('Ingrese dP',min_value=0.00, max_value=None, value=1.00,  step=0.1)
				m =abs((p2-p1)/(t2-t1))
				st.write('m:', m )
			with col9 :
		...
		with st.beta_expander('Pressure Drawdown Test Sequence  Gráficos comninados '):
			...
			ax.plot(df_limitado['t, hr'], df_limitado['pwf, psia'],marker = '^',label="pwf, psia")
		...
		with st.beta_expander('Pressure Drawdown Test Sequence '):
			...
			with col3:
				...
				ax.plot(df_limitado['t, hr'], df_limitado['pwf, psia'] , marker = 'x')
				plt.xscale('log')
				plt.xlim(0.1, 1000)
